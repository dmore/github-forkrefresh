...
// var logfile
var keychain_app_service = "github-forkrefresh"
var keychain_username = "dmore"
...
func main() {
	...
	var keychain_app_service_tmp = os.Getenv("KEYCHAIN_APP_SERVICE")
	if len(keychain_app_service_tmp) == 0 {
		//log.Fatal(err)
		//os.Exit(1)
		log.Println("KEYCHAIN_APP_SERVICE not set. using default : " + keychain_app_service)
	} else {
		keychain_app_service = keychain_app_service_tmp
	}
	log.Println("KEYCHAIN_APP_SERVICE:" + keychain_app_service)

	//keychain username from env var
	var keychain_username_tmp = os.Getenv("KEYCHAIN_USERNAME")
	if len(keychain_username_tmp) == 0 {
		//log.Fatal(err)
		//os.Exit(1)
		log.Println("KEYCHAIN_USERNAME not set. using default : " + keychain_username)
	} else {
		keychain_username = keychain_username_tmp
	}
